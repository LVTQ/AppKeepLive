...
import cn.tklvyou.appkeeplive.keeplive.onepx.ScreenReceiver;
import cn.tklvyou.appkeeplive.keeplive.timer.ScheduleService;


/**
 * 在 Service 启动的时候，开启一个定时任务：
 * 每十五分钟检查一次 Service 是否存活，如果未在运行，就启动该 Service
 */
...
//    private final static long WAKE_INTERVAL = 15 * 60 * 1000;
    private final static long WAKE_INTERVAL = 5000;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                //setPeriodic方法在Android 5.0和6.0系统能够正常运行.但是在Android7.0不能正常工作的解决方案。
                if (Build.VERSION.SDK_INT >= 24) {
                    builder.setMinimumLatency(WAKE_INTERVAL); //执行的最小延迟时间
                    builder.setOverrideDeadline(WAKE_INTERVAL);  //执行的最长延时时间
                    builder.setMinimumLatency(WAKE_INTERVAL);
                    builder.setBackoffCriteria(WAKE_INTERVAL, JobInfo.BACKOFF_POLICY_LINEAR);//线性重试方案
